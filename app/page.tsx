...
import { Barcode, Boxes, Building2, CalendarDays, Camera, CheckCircle2, Check, ChevronRight, CircleDollarSign, ClipboardCheck, Clock3, Download, Eye, EyeOff, FileSpreadsheet, Hand, LoaderCircle, LogOut, Mail, PackageSearch, Plus, RefreshCw, Ruler, ScanLine, ShieldCheck, Store, Tags, TriangleAlert, Upload, UserRound, UserPlus, Users, X } from "lucide-react";
...
function LoginRedirect() {
  ...
  return <main className="loading-screen"><Image src="/canaima-logo.svg" alt="Grupo Canaima" width={480} height={250} priority/><LoaderCircle className="spin" size={26}/><span>Validando acceso…</span></main>;
}
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    supabase.auth.getClaims().then(({ data, error })=>{
      applySession(!error && typeof data?.claims?.sub === "string" ? data.claims.sub : null);
    });
  ...
  async function signOut(){stopCamera();setLastProduct(null);setScanFeedback(null);setEvaluationItems([]);await supabase.auth.signOut();window.location.replace("/login");}
  ...
  if(!sessionUserId)return <LoginRedirect/>;
