...
import { Camera, CheckCircle2, Check, ChevronRight, Clock3, Download, Eye, EyeOff, KeyRound, LoaderCircle, LogIn, LogOut, Mail, Plus, RefreshCw, ShieldCheck, Store, TriangleAlert, Upload, UserRound, UserPlus, X } from "lucide-react";
...
import { TechnicalIcon, type TechnicalIconKind } from "@/app/technical-icon";
...
function NavItem({ icon, label, active, onClick }: { icon: TechnicalIconKind; label: string; active: boolean; onClick: () => void }) {
  return <button className={`nav-item ${active ? "nav-item-active" : ""}`} onClick={onClick} type="button"><TechnicalIcon kind={icon} size={20}/><span>{label}</span><ChevronRight className="nav-chevron" size={16}/></button>;
}
...
function ExcelDocumentIcon({ size = "large" }: { size?: "large" | "small" }) {
  return <span className={`excel-document-icon excel-document-icon-${size}`} aria-hidden="true">
    <TechnicalIcon kind="inventory" size={size === "small" ? 34 : 50}/>
    <span className="excel-document-label">XLS</span>
  </span>;
}

function EvaluationSummaryIcon({ observation }: { observation?: Observation }) {
  const kind: TechnicalIconKind = observation === "SIN INCIDENCIAS" ? "success" : observation === "PRECIO ERRÓNEO" ? "price" : observation === "MAL ETIQUETADO" ? "tag" : observation === "TALLA MENOR NO EXHIBIDA" ? "size" : "unlabeled";
  return <span className={`summary-icon summary-icon-${observation === "SIN INCIDENCIAS" ? "success" : "default"}`} aria-hidden="true"><TechnicalIcon kind={kind} size={20}/></span>;
}
...
function LoginScreen() {
  ...
  const signupCities = [...new Set(signupStores.map((store)=>store.city || "Otras tiendas"))].sort((a,b)=>a.localeCompare(b,"es"));
  const cityStores = signupStores.filter((store)=>(store.city || "Otras tiendas")===signupCity);
  ...
  async function openSignup() {
    ...
    const { data, error } = await supabase.rpc("registration_stores_by_city");
    if (error) setSignupMessage({ kind:"error", text:"El registro por tienda todavía debe activarse en Supabase." });
      ...
      setSignupForm((current)=>({...current,storeId:""}));
      setSignupCity("");
    }
  ...
  async function submitSignup(event:FormEvent<HTMLFormElement>) {
    ...
    if(!fullName||!signupEmail||signupForm.password.length<8||!signupForm.storeId||!cityStores.some((store)=>store.id===signupForm.storeId)){
    ...
    <Dialog open={signupOpen} onOpenChange={(open)=>!signupBusy&&setSignupOpen(open)}>
      <DialogContent className="user-dialog signup-dialog">
        ...
        <form className="create-user-form" onSubmit={submitSignup}>
          ...
          <label>Ciudad<Select value={signupCity} disabled={signupStoresLoading||!signupStores.length} onValueChange={(value)=>{setSignupCity(value);setSignupForm({...signupForm,storeId:""});}}><SelectTrigger><SelectValue placeholder="Seleccionar ciudad"/></SelectTrigger><SelectContent>{signupCities.map((city)=><SelectItem key={city} value={city}>{city}</SelectItem>)}</SelectContent></Select></label>
          <label>Tienda asignada<Select value={signupForm.storeId} disabled={signupStoresLoading||!signupCity||!cityStores.length} onValueChange={(value)=>setSignupForm({...signupForm,storeId:value})}><SelectTrigger><SelectValue placeholder={signupStoresLoading?"Cargando tiendas…":"Seleccionar tienda"}/></SelectTrigger><SelectContent>{cityStores.map((store)=><SelectItem key={store.id} value={store.id}>{store.name}</SelectItem>)}</SelectContent></Select></label>
...
export default function Home() {
  ...
  useEffect(()=>{
    if(view!=="catalog"||uploading||transfersBusy)return;
  ...
  const incidentItems=useMemo(()=>evaluationItems.filter((item)=>item.observation!=="SIN INCIDENCIAS"),[evaluationItems]);
  ...
  async function exportEvaluation(){
    const [{createEvaluationReport},{Packer}]=await Promise.all([import("@/app/lib/evaluation-report"),import("docx")]);
    const doc=createEvaluationReport(currentStore?.name??"Tienda",evaluationItems);
  ...
  return <div className="app-shell"><Toaster position="top-center" richColors/>
    ...
    <aside className={`sidebar ${mobileMenu?"sidebar-open":""}`}>
      ...
      <nav className="nav-list"><NavItem icon="scan" label="Escanear producto" active={view==="scanner"} onClick={()=>goTo("scanner")}/>{isEvaluator&&<NavItem icon="evaluation" label="Evaluación" active={view==="evaluation"} onClick={()=>goTo("evaluation")}/>} {canViewDaily&&<NavItem icon="register" label="Registro diario" active={view==="daily"} onClick={()=>goTo("daily")}/>}<NavItem icon="inventory" label="Inventario" active={view==="catalog"} onClick={()=>goTo("catalog")}/>{isOwner&&<NavItem icon="users" label="Usuarios y permisos" active={view==="users"} onClick={()=>goTo("users")}/>}</nav>
      <div className="sidebar-store"><div className="store-mark"><TechnicalIcon kind="store" size={18}/></div><div><span>Tienda activa</span><strong>{currentStore?.name??"Seleccionar tienda"}</strong></div></div><button className="sidebar-user" type="button" onClick={signOut}><div className="avatar">{initials}</div><div><strong>{displayName}</strong><span>{roleLabel}</span></div><LogOut size={18}/></button>
    </aside>
    <main className="workspace">
      ...
      {view==="evaluation"&&isEvaluator&&<section className="page-content evaluation-page">
        ...
        <div className="summary-grid"><div className="summary-total"><span className="summary-icon" aria-hidden="true"><TechnicalIcon kind="evaluation" size={20}/></span><span>EVALUADOS</span><strong>{evaluationItems.length}</strong></div>{summary.map((item)=><div key={item.observation}><EvaluationSummaryIcon observation={item.observation}/><span>{item.observation}</span><strong>{item.count}</strong></div>)}</div>
      ...
      {view==="daily"&&canViewDaily&&<section className="page-content daily-page">
        <div className="daily-header">
          <div><Badge variant="outline"><TechnicalIcon kind="register" size={14}/> Control operativo</Badge><h2>Registro diario</h2><p>Resumen de fallos por tienda, con el detalle completo disponible cuando lo necesites.</p></div>
        ...
        {dailyError?<div className="daily-setup"><TriangleAlert size={25}/><div><strong>Registro diario pendiente de activación</strong><p>{dailyError}</p></div></div>:<>
          <div className="daily-summary-grid"><div><span><TechnicalIcon kind="scan" size={20}/></span><small>ESCANEOS</small><strong>{dailySummary.totalScans}</strong></div><div className="daily-summary-incidents"><span><TechnicalIcon kind="alert" size={20}/></span><small>FALLOS ENCONTRADOS</small><strong>{dailySummary.incidents}</strong></div><div><span><TechnicalIcon kind="price" size={20}/></span><small>PRECIO ERRÓNEO</small><strong>{dailySummary.priceErrors}</strong></div><div><span><TechnicalIcon kind="tag" size={20}/></span><small>MAL ETIQUETADO</small><strong>{dailySummary.mislabeled}</strong></div><div><span><TechnicalIcon kind="unlabeled" size={20}/></span><small>SIN ETIQUETA</small><strong>{dailySummary.withoutLabel}</strong></div><div><span><TechnicalIcon kind="size" size={20}/></span><small>TALLA MENOR NO EXHIBIDA</small><strong>{dailySummary.smallerSizeNotDisplayed}</strong></div></div>
    ...
    <nav className="mobile-bottom-nav" aria-label="Navegación principal">
      <button className={view==="scanner"?"is-active":""} aria-current={view==="scanner"?"page":undefined} onClick={()=>goTo("scanner")} type="button"><TechnicalIcon kind="scan" size={21}/><span>Escanear</span></button>
      {isEvaluator&&<button className={view==="evaluation"?"is-active":""} aria-current={view==="evaluation"?"page":undefined} onClick={()=>goTo("evaluation")} type="button"><TechnicalIcon kind="evaluation" size={21}/><span>Evaluación</span></button>}
      {canViewDaily&&<button className={view==="daily"?"is-active":""} aria-current={view==="daily"?"page":undefined} onClick={()=>goTo("daily")} type="button"><TechnicalIcon kind="register" size={21}/><span>Registro</span></button>}
      <button className={view==="catalog"?"is-active":""} aria-current={view==="catalog"?"page":undefined} onClick={()=>goTo("catalog")} type="button"><TechnicalIcon kind="inventory" size={21}/><span>Inventario</span></button>
      {isOwner&&<button className={view==="users"?"is-active":""} aria-current={view==="users"?"page":undefined} onClick={()=>goTo("users")} type="button"><TechnicalIcon kind="users" size={21}/><span>Usuarios</span></button>}
    </nav>
  </div>;
}
