...
import { ArrowRightLeft, Barcode, Boxes, Building2, CalendarDays, Camera, CheckCircle2, Check, ChevronRight, CircleDollarSign, ClipboardCheck, Clock3, Download, Eye, EyeOff, FileSpreadsheet, Hand, KeyRound, LoaderCircle, LogIn, LogOut, Mail, PackageSearch, Plus, RefreshCw, Ruler, ScanLine, ShieldCheck, Store, Tags, TriangleAlert, Upload, UserRound, UserPlus, Users, X } from "lucide-react";
...
export default function Home() {
  ...
  return <div className="app-shell"><Toaster position="top-center" richColors/>
    ...
    <aside className={`sidebar ${mobileMenu?"sidebar-open":""}`}>
      ...
      <nav className="nav-list"><NavItem icon={ScanLine} label="Escanear producto" active={view==="scanner"} onClick={()=>goTo("scanner")}/>{isEvaluator&&<NavItem icon={ClipboardCheck} label="Evaluación" active={view==="evaluation"} onClick={()=>goTo("evaluation")}/>} {canViewDaily&&<NavItem icon={CalendarDays} label="Registro diario" active={view==="daily"} onClick={()=>goTo("daily")}/>}<NavItem icon={FileSpreadsheet} label="Inventario" active={view==="catalog"} onClick={()=>goTo("catalog")}/>{isOwner&&<NavItem icon={Users} label="Usuarios y permisos" active={view==="users"} onClick={()=>goTo("users")}/>}</nav>
    ...
    <nav className="mobile-bottom-nav" aria-label="Navegación principal">
      ...
      <button className={view==="catalog"?"is-active":""} aria-current={view==="catalog"?"page":undefined} onClick={()=>goTo("catalog")} type="button"><FileSpreadsheet size={21}/><span>Inventario</span></button>
